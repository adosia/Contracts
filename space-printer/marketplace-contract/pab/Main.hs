...
instance Builtin.HasDefinitions StarterContracts where
    getDefinitions = [MarketplaceContract]
    getSchema =  \case
        MarketplaceContract -> Builtin.endpointsToSchemas @MarketplaceContract.Schema
    getContract = \case
        MarketplaceContract -> SomeBuiltin (MarketplaceContract.contract @ContractError)
